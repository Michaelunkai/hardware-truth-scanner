...
$smartPredictData = @(Invoke-OptionalProbe "smart predict data" { Get-CimInstance -Namespace root\wmi -ClassName MSStorageDriver_FailurePredictData })
$smartPredictThresholds = @(Invoke-OptionalProbe "smart predict thresholds" { Get-CimInstance -Namespace root\wmi -ClassName MSStorageDriver_FailurePredictThresholds })
$smartScsiInfoExceptions = @(Invoke-OptionalProbe "smart scsi info exceptions" { Get-CimInstance -Namespace root\wmi -ClassName MSStorageDriver_ScsiInfoExceptions })
$smartAtapiData = @(Invoke-OptionalProbe "smart atapi data" { Get-CimInstance -Namespace root\wmi -ClassName MSStorageDriver_ATAPISmartData })
...
$smartProviderProbeErrors = @($optionalProbeErrors | Where-Object { $_.Name -match "smart" })
$smartProviderRows = @(
  [pscustomobject]@{ Provider = "MSStorageDriver_FailurePredictStatus"; Rows = $smartStatus.Count }
  [pscustomobject]@{ Provider = "MSStorageDriver_FailurePredictData"; Rows = $smartPredictData.Count }
  [pscustomobject]@{ Provider = "MSStorageDriver_FailurePredictThresholds"; Rows = $smartPredictThresholds.Count }
  [pscustomobject]@{ Provider = "MSStorageDriver_ScsiInfoExceptions"; Rows = $smartScsiInfoExceptions.Count }
  [pscustomobject]@{ Provider = "MSStorageDriver_ATAPISmartData"; Rows = $smartAtapiData.Count }
)
$smartProviderRowCount = ($smartProviderRows | Measure-Object -Property Rows -Sum).Sum
$smartProviderSignals = @()
if ($smartProviderRowCount -eq 0) { $smartProviderSignals += "Windows WMI SMART provider classes returned no attribute or threshold rows to this scanner." }
if ($smartProviderProbeErrors.Count -gt 0) { $smartProviderSignals += "$($smartProviderProbeErrors.Count) SMART provider attempt(s) returned errors or unsupported responses." }
New-Component -Category "Storage SMART Provider Coverage" -Name "SMART WMI provider availability and attribute coverage" -Status $(if ($smartPredicting.Count -gt 0) { "critical" } elseif ($smartProviderRowCount -gt 0) { "ok" } else { "unknown" }) -Confidence "medium" -Evidence @{
  ProviderRows = $smartProviderRows
  ProviderErrors = @($smartProviderProbeErrors | Select-Object -First 20 Name, Error)
  PredictingFailureCount = $smartPredicting.Count
} -Signals $smartProviderSignals -Recommendations $(if ($smartPredicting.Count -gt 0) { @("Back up immediately and replace the affected drive after vendor confirmation.") } elseif ($smartProviderRowCount -gt 0) { @("Windows exposed SMART provider rows. Use vendor tools or smartctl for full attribute interpretation before replacing hardware.") } else { @("Windows did not expose SMART attribute/threshold providers here. Use each SSD/HDD vendor diagnostic or smartctl from a trusted package for drive-internal certainty.") })
...
$sensorProviderErrors = @($optionalProbeErrors | Where-Object { $_.Name -match "thermal zones|openhardwaremonitor|librehardwaremonitor|sensor monitor commands" })
$sensorNamespaceNames = @($sensorRootNamespaces | ForEach-Object { [string]$_ })
$sensorToolRows = @($sensorToolCommands | Select-Object -First 20 Name, Source, Version)
$sensorProviderSignals = @()
if ($thermalZones.Count -eq 0) { $sensorProviderSignals += "ACPI thermal-zone provider returned no sensor rows." }
if ($fans.Count -eq 0) { $sensorProviderSignals += "Win32_Fan returned no fan sensor rows." }
if ($thirdPartySensors.Count -eq 0) { $sensorProviderSignals += "OpenHardwareMonitor/LibreHardwareMonitor did not expose live sensor rows." }
if ($sensorToolRows.Count -eq 0) { $sensorProviderSignals += "No common sensor or smartctl executable was found on PATH." }
if ($sensorProviderErrors.Count -gt 0) { $sensorProviderSignals += "$($sensorProviderErrors.Count) sensor-provider attempt(s) returned errors or unsupported responses." }
New-Component -Category "Sensor Provider Coverage" -Name "Thermal, fan, and monitoring-provider availability" -Status $(if ($hotThirdPartySensors.Count -gt 0) { "warning" } elseif ($thermalZones.Count -gt 0 -or $fans.Count -gt 0 -or $thirdPartySensors.Count -gt 0) { "ok" } else { "unknown" }) -Confidence "medium" -Evidence @{
  RootNamespaces = @($sensorNamespaceNames | Select-Object -First 40)
  HasOpenHardwareMonitorNamespace = $sensorNamespaceNames -contains "OpenHardwareMonitor"
  HasLibreHardwareMonitorNamespace = $sensorNamespaceNames -contains "LibreHardwareMonitor"
  AcpiThermalZoneRows = $thermalZones.Count
  WmiFanRows = $fans.Count
  ThirdPartySensorRows = $thirdPartySensors.Count
  DetectedToolCommands = $sensorToolRows
  ProviderErrors = @($sensorProviderErrors | Select-Object -First 20 Name, Error)
} -Signals $sensorProviderSignals -Recommendations $(if ($hotThirdPartySensors.Count -gt 0) { @("Inspect cooling immediately and validate with BIOS/vendor tools.") } elseif ($thermalZones.Count -gt 0 -or $fans.Count -gt 0 -or $thirdPartySensors.Count -gt 0) { @("At least one sensor provider returned rows. Use vendor or BIOS readings for final fan, pump, VRM, and thermal-paste certainty.") } else { @("Windows did not expose usable thermal/fan sensors through the checked providers. Use BIOS/UEFI, motherboard/GPU vendor tools, or physical inspection for cooling certainty.") })
...
$kernelPowerInformationalEvents = @($events | Where-Object { $_.ProviderName -eq "Microsoft-Windows-Kernel-Power" -and $_.LevelDisplayName -notmatch "Critical|Error|Warning" -and $_.Id -ne 41 })
$volmgrProblemEvents = @($events | Where-Object { $_.ProviderName -eq "volmgr" -and $_.LevelDisplayName -match "Critical|Error|Warning" })
$powerStabilityProblemEvents = @($kernelPowerProblemEvents + $unexpectedShutdownEvents + $volmgrProblemEvents | Where-Object { $_ })
New-Component -Category "Power Stability" -Name "PSU, AC, battery, and unexpected shutdown evidence" -Status $(if ($powerStabilityProblemEvents.Count -gt 0) { "warning" } else { "ok" }) -Confidence "medium" -Evidence @{
  ...
  KernelPowerInformationalEventCount = $kernelPowerInformationalEvents.Count
  UnexpectedShutdownEventCount = $unexpectedShutdownEvents.Count
  VolmgrProblemEventCount = $volmgrProblemEvents.Count
  ...
  InformationalKernelPowerSamples = @($kernelPowerInformationalEvents | Select-Object -First 10 TimeCreated, Id, ProviderName, LevelDisplayName, @{ Name = "Message"; Expression = { Convert-ToBoundedText $_.Message 350 } })
} -Signals $(if ($powerStabilityProblemEvents.Count -gt 0) { @("$($powerStabilityProblemEvents.Count) recent power-loss, unexpected-shutdown, or volmgr crash-path problem event(s) were found.") } else { @() }) -Recommendations $(if ($powerStabilityProblemEvents.Count -gt 0) { @("Correlate these timestamps with outages, freezes, sleep/resume, GPU load, CPU load, wall power, PSU cables, crash-dump configuration, and UPS behavior before replacing parts.") } else { @("No recent Kernel-Power 41, EventLog 6008, or warning/error volmgr crash-path evidence was found in the scan window. Informational Kernel-Power reboot/shutdown entries are preserved as context, not hardware faults. PSU ripple, weak cables, wall power, and load-only failures still require controlled physical/load testing.") })
...
New-Diagnostic -Name "Power source and unexpected shutdown sweep" -Status $(if ($powerStabilityProblemEvents.Count -gt 0) { "warning" } else { "passed" }) -Evidence "$($powerSupplies.Count) power supply row(s), $($upsDevices.Count) UPS row(s), $($batteries.Count) battery row(s), $($kernelPowerProblemEvents.Count) Kernel-Power problem event(s), $($kernelPowerInformationalEvents.Count) Kernel-Power informational event(s), $($unexpectedShutdownEvents.Count) EventLog 6008 unexpected shutdown event(s), $($volmgrProblemEvents.Count) volmgr warning/error event(s)." -NextStep $(if ($powerStabilityProblemEvents.Count -gt 0) { "Correlate power-event timestamps with wall power, UPS, PSU, sleep/resume, GPU/CPU load, crash-dump configuration, and cable seating." } else { "No recent Windows power-loss/crash-path problem evidence was found; informational reboot/shutdown entries are not hardware faults. PSU and wall-power quality still require physical/load testing if symptoms exist." })
...
New-Diagnostic -Name "Sensor provider availability sweep" -Status $(if ($hotThirdPartySensors.Count -gt 0) { "warning" } elseif ($thermalZones.Count -gt 0 -or $fans.Count -gt 0 -or $thirdPartySensors.Count -gt 0) { "passed" } else { "limited" }) -Evidence "$($sensorRootNamespaces.Count) root namespace row(s), $($sensorToolRows.Count) PATH tool command(s), $($sensorProviderErrors.Count) provider error/unsupported response(s)." -NextStep $(if ($thermalZones.Count -gt 0 -or $fans.Count -gt 0 -or $thirdPartySensors.Count -gt 0) { "Use returned sensor rows plus BIOS/vendor tools for final cooling certainty." } else { "No usable thermal/fan provider was exposed; use BIOS/UEFI, motherboard/GPU vendor tools, or physical inspection." })
