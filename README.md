...
- Checks Device Manager problem codes, storage health, SMART failure prediction, SMART provider/attribute availability, storage reliability counters, disk-to-partition-to-volume mapping, per-physical-drive storage risk summaries, storage controller/adapter PnP inventory, live disk queue/throughput/free-space counters, filesystem dirty-bit status, bounded read-only dirty-volume `chkdsk` evidence, recent WHEA/disk/NTFS/display/power/PnP events with provider-level coverage, power-source and unexpected-shutdown stability evidence, Reliability Monitor and Windows Error Reporting hardware-crash records, Windows crash dump artifacts with bounded keyword hint extraction, CPU topology/cache/virtualization and live performance-counter evidence, GPU telemetry through `nvidia-smi` when present, GPU display-mode/VRAM/driver/link telemetry, safe DirectX-adjacent inventory with opt-in `dxdiag`, thermal/fan telemetry and sensor-provider availability when Windows or OpenHardwareMonitor/LibreHardwareMonitor exposes it, installed vendor/SMART/sensor/memory diagnostic tool availability, Windows Memory Diagnostic history, DIMM slot topology and memory error-correction metadata, live RAM commit/pagefile/paging pressure counters, PCI/PCIe inventory, PCIe root/switch port and lane-sensitive device topology, firmware-class devices, driver-signing inventory with `pnputil` package-signer cross-checks, USB/input/monitor devices, USB/USBSTOR attached-device inventory, USB controller-to-device topology, EDID display identity and connection metadata, HID/Bluetooth/camera/sensor/printer peripheral inventory, network link speed and packet error/discard counters, audio endpoint/media device inventory, power capabilities, RAM inventory, battery status, network adapters, audio devices, volumes, and an explicit remaining proof-gap checklist for anything live software cannot prove.
