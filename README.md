...
- Checks Device Manager problem codes, storage health, SMART failure prediction, storage reliability counters, recent WHEA/disk/NTFS/display/power events, GPU telemetry through `nvidia-smi` when present, DirectX diagnostics, thermal/fan telemetry when Windows exposes it, driver-signing inventory, USB/input/monitor devices, power capabilities, RAM inventory, battery status, network adapters, audio devices, and volumes.
